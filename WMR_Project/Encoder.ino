void Encoder_Setup(){
  ...
  #ifdef BT_DEBUG
  Serial3.println("Initializing encoder...");
  #endif
 //Parameter setup
 digitalWrite(LED_PING,true);
 digitalWrite(LED_PINY,true);
 digitalWrite(LED_PINR,true); 
 Tc* pTc[] = { TC0, TC2 };
 ...
 uint32_t ulPin = 0; 
  ...
  pmc_enable_periph_clk( TC_INTERFACE_ID );    // TC0 Channel0
  pmc_enable_periph_clk( TC_INTERFACE_ID + 2); // TC0 Channel2
  pmc_enable_periph_clk( TC_INTERFACE_ID + 6 ); // TC2 Channel0
  pmc_enable_periph_clk( TC_INTERFACE_ID + 8 ); // TC2 Channel2
...
//  pmc_enable_periph_clk( TC_INTERFACE_ID + 3*i);   // Channel 0
//  pmc_enable_periph_clk( TC_INTERFACE_ID + 3*i+2); // Channel 2
  
  // Enable qua mode  
  pTc[i]->TC_BMR = TC_BMR_QDEN | TC_BMR_SPEEDEN 
                 | TC_BMR_FILTER | TC_BMR_MAXFILT_Msk;
  // Set register for each Channel
  // Channel0 works at caputure mode, and the clock is generated by each edge
  ...
  // sensors, so it just need a general setup to fit the artecture of this
  // mode  
  pTc[i]->TC_CHANNEL[1].TC_CMR = TC_CMR_TCCLKS_XC0;
  // The Clock Frequency of Channel 2 is MCK/32 = 2625KHZ, When the conter
  // reach 5250 counts, it will triger the channel0 and clear the counter
  // in Channel0  
  pTc[i]->TC_CHANNEL[2].TC_CMR = TC_CMR_TCCLKS_TIMER_CLOCK3 
                               | TC_CMR_WAVE| TC_CMR_WAVSEL_UP_RC 
                               | TC_CMR_ACPC_TOGGLE;
  TC_SetRC( pTc[i], 2, 26250);
